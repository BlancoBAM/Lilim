...
if [ -f "$HOST_SCRIPT_PATH" ]; then
  echo "[INFO] Host script $HOST_SCRIPT_PATH already exists; skipping generation to keep changes idempotent."
else
...
chmod +x ~/lilim_host_apply_and_test.sh
echo "Created: ~/lilim_host_apply_and_test.sh"
echo "Run with: REPO_DIR=/your/path BRANCH=your-branch RUN_PUSH=1 ~/lilim_host_apply_and_test.sh"
