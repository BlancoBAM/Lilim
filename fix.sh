...
# This script generates an enhanced, idempotent Lilim host workflow script:
#   ~/lilim_host_apply_and_test.sh
# The generated script is designed to be safe to re-run, with logging,
# proper state checks, and robust error handling.
...
HOST_SCRIPT_PATH="${HOST_SCRIPT_PATH:-$HOME/lilim_host_apply_and_test.sh}"
cat > "$HOST_SCRIPT_PATH" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
sudo apt-get install -y \
  ...
  curl jq git rsync \
  build-essential rpm dpkg-dev \
  libwebkit2gtk-4.1-dev libappindicator3-dev librsvg2-dev patchelf \
  pkg-config libssl-dev || true
...
pip install --upgrade fastapi uvicorn pydantic litellm apscheduler pytest
...
if [ -d "tests" ]; then
  python -m pytest tests/ -v || python -m unittest discover -s tests -v || true
else
...
for f in routing.toml lilim-identity.json; do
...
sudo install -m 0644 systemd/system/lilith-ai.service /etc/systemd/system/lilith-ai.service || true
sudo mkdir -p /var/backups/lilim

# Install lilim-runtime binary manually if deb didn't
if [ -f "crates/lilim-runtime/target/release/lilim-runtime" ]; then
  sudo install -m 0755 crates/lilim-runtime/target/release/lilim-runtime /usr/bin/lilim-runtime
fi
...
sudo systemctl --no-pager --full status lilith-ai.service | head -n 20 || true
echo "--- LOG TAIL ---"
sudo journalctl -u lilith-ai.service -n 60 --no-pager || true
...
echo "Created/Updated: $HOST_SCRIPT_PATH"
echo "Run with: REPO_DIR=/your/path BRANCH=your-branch RUN_PUSH=1 $HOST_SCRIPT_PATH"
